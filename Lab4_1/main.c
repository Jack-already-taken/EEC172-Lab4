...
char prevSampleResult = 0;
int sampleCount = 0;

long power_all[8];       // array to store calculated power of 8 frequencies
long threshold = 90000;
...
void buttonPress()
{
    ...
    if (sampleResult != prevSampleResult) {
        ...
        if (sampleResult != NONE) {
            prevSampleResult = sampleResult;
            sampleCount = 1;
        }
    }
    else if (sampleResult != NONE) {
        sampleCount++;
    }
